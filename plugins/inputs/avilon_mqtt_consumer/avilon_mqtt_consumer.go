...
type AvilonMQTTConsumer struct {
	Servers                []string
	Topics                 []string
	Username               string
	Password               string
...
func (m *AvilonMQTTConsumer) SampleConfig() string {
	return sampleConfig
}

func (m *AvilonMQTTConsumer) Description() string {
	return "Read metrics from Avilon MQTT topic(s)"
}

func (m *AvilonMQTTConsumer) SetParser(parser parsers.Parser) {
	m.parser = parser
}

func (m *AvilonMQTTConsumer) Start(acc telegraf.Accumulator) error {
...
func (m *AvilonMQTTConsumer) connect() error {
...
func (m *AvilonMQTTConsumer) onConnectionLost(c mqtt.Client, err error) {
...
func (m *AvilonMQTTConsumer) recvMessage(c mqtt.Client, msg mqtt.Message) {
	for {
		select {
...
func (m *AvilonMQTTConsumer) onMessage(acc telegraf.TrackingAccumulator, msg mqtt.Message) error {
	Layers := strings.Split(msg.Topic(), "/")
	...
	//Payload json
	buf := bytes.TrimSpace(msg.Payload())
		...
		for k, v := range jsonOut {
			...
			case float64:
				if k == "delay" {
					...
					nTime = nTime.Add(-1 * time.Millisecond * time.Duration(v.(float64)))
				} else if k != "timestamp" {
					field[k] = v
				}
			case string:
				if k == "timestamp" {
					...
					if err != nil {
						err = fmt.Errorf("unable to parse string to timestamp, %s", err)
						return err
					}
	...
	met, err := metric.New(Layers[1], tags, field, nTime, telegraf.Untyped)
	if err != nil {
		return err
	}

	id := acc.AddTrackingMetric(met)
	m.messages[id] = true
	return nil
}

func (m *AvilonMQTTConsumer) Stop() {
...
func (m *AvilonMQTTConsumer) Gather(acc telegraf.Accumulator) error {
...
func (m *AvilonMQTTConsumer) createOpts() (*mqtt.ClientOptions, error) {
...
func init() {
	inputs.Add("avilon_mqtt_consumer", func() telegraf.Input {
		return &AvilonMQTTConsumer{
